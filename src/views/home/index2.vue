<template>
  <div class="home-container">
    ...
    <!-- 频道列表 -->
    <van-tabs v-model="active" class="channel-tabs" animated swipeable sticky>
    ...
    <van-popup
      ...
      :style="{ height: '100%' }"
    >
      <!-- 打开频道编辑时，把当前用户的频道数据channels传递给channel-edit组件 -->
      <ChannelEdit :active="active" :my-channels="channels" @update-active="updateActive" />
      <!-- 使用大驼峰形式的名字注册的组件，使用时可以使用连字符形式 -->
...
export default {
  ...
  data() {
    return {
      active: 0, // 标识 当前高亮的频道项的索引
      channels: [],
      isChannelEditShow: false // 控制 编辑弹层是否显示
    }
  },
  created() {
